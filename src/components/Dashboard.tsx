...
import {
  ...
  Calendar, TrendingUp, LogOut, Moon, Filter, Bell, Sparkles, Key, StickyNote
} from 'lucide-react';
...
type View = 'dashboard' | 'analytics' | 'profile' | 'calendar' | 'apiManager' | 'notes';
...
export default function Dashboard() {
  ...
  if (view === 'profile') {
    return (
      <UserProfile 
        ...
        onSubscriptionsUpdate={loadSubscriptions}
      />
    );
  }
  ...
  if (view === 'notes') {
    return <NotesView onBack={() => setView('dashboard')} />;
  }
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-slate-200 py-1 z-50 animate-scale-in origin-top-right">
                    ...
                    <div className="py-1">
                      ...
                      <MenuButton icon={StickyNote} onClick={() => { setView('notes'); setShowUserMenu(false); }}>
                        {t('nav.notes') || 'Notes'}
                      </MenuButton>
                    </div>
